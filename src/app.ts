...
class Game {
   ...
   */
  public rocketFactory(name: string, type: string): any {
    let xPosition = GameItem.randomInteger(0, this.canvas.width - 200);
    let yPosition = GameItem.randomInteger(0, this.canvas.height - 200);
    let image: HTMLImageElement;

    if (type === 'leftToRight') {
      xPosition = 0;
      image = GameItem.loadNewImage('./assets/rocket-horizontal.png');
    } else {
      yPosition = 0;
      image = GameItem.loadNewImage('./assets/rocket-vertical.png');
    }
    ...
    return {
      ...
      yPos: yPosition,
      type: type,
      speed: GameItem.randomInteger(0, 15),
      image: image,
    };
  }
}
